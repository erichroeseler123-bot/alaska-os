...
export async function GET(request: NextRequest) {
  ...
  try {
    if (!fs.existsSync(MASTER_FILE)) throw new Error("Master data file missing.");
    const rawFile = await fs.readFile(MASTER_FILE, 'utf-8');
    const allTours = JSON.parse(rawFile).items;
    ...
    const results = await Promise.all(
      CONFIG.PORTS.map(async (port) => {
        ...
        // Save to Redis & Public JSON
        await redis.set(`static_tours_fallback:${port.slug}`, portPayload);
        await fs.ensureDir(PUBLIC_API_DIR);
        await fs.writeJSON(path.join(PUBLIC_API_DIR, `${port.slug}.json`), portPayload, { spaces: 2 });

        return { port: port.slug, status: 'Updated' };
      })
    );
