...
import { CONFIG } from '../../../lib/config'; // Contains your PORTS list with lat/lon
import { fetchWeather } from '../../../lib/providers/weather'; // Your existing provider
  ...
  try {
    ...
    const results = await Promise.all(
      CONFIG.PORTS.map(async (port) => {
        ...
        // A. Fetch Live Weather (Using your existing provider)
        ...
        const portPayload = {
            ...
            // The Weather Data (Safe fallback if provider fails)
            weather: weatherData.ok ? weatherData.current : { tempF: "N/A", condition: "Unavailable" },
            weatherForecast: weatherData.ok ? weatherData.forecast : [],

            // The Tours
            tours: portTours
        };

        // D. Save to Redis (Hot Cache)
        await redis.set(`static_tours_fallback:${port.slug}`, portPayload);

        // E. Save to Public JSON API (Static CDN)
        ...
        return { port: port.slug, tours: portTours.length, weather: weatherData.ok ? 'OK' : 'FAIL' };
      })
    );
